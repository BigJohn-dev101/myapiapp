  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-red-900 via-yellow-700 to-orange-800 py-20">
      <div className="bg-white rounded-2xl shadow-lg p-10 w-full max-w-xl">
        ...
        {submitted ? (
          <div className="text-green-700 font-semibold mb-4">Claim submitted successfully!</div>
        ) : null}
        <form onSubmit={handleSubmit} className="space-y-6">
          <input name="claimID" value={form.claimID} onChange={handleChange} placeholder="Claim ID" className="w-full px-4 py-2 border rounded" required />
          ...
          <button type="submit" className="w-full bg-orange-800 text-white py-3 rounded font-bold hover:bg-orange-600 transition">Submit Claim</button>
        </form>
      </div>
    </main>
  );
